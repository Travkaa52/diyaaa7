import React from 'react';
import Head from 'next/head';
import Link from 'next/link';

// Ця сторінка буде доступна за адресою http://localhost:3000/home
export default function HomePage() {
  return (
    <>
      {/* Налаштування тегів <head> для цієї сторінки */}
      <Head>
        <title>Стрічка | Дія</title>
        <meta name="theme-color" content="#ffffff" />
        <link rel="manifest" href="/manifest.json" />
        <link rel="icon" href="/icon.png" />
        <link rel="apple-touch-icon" href="/icon.png" />
      </Head>

      {/* Основний вміст сторінки (конвертований <body>) */}
      <main className="min-h-screen max-h-screen bg-gradient-to-b from-[#7AC7C0] via-[#B8D7EA] to-[#C5B6E0] pb-[120px] pt-14 overflow-y-auto">
        <div className="px-5 flex flex-col gap-6">
          <h1 className="text-[32px] font-bold flex items-center gap-2">
            Привіт, {/* */}Користувач {/* */}
            <span className="text-2xl">👋</span>
          </h1>
          
          {/* Блок Незламність */}
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-5 flex flex-col gap-2 relative group cursor-default">
            <h2 className="text-2xl font-semibold">Незламність</h2>
            <p className="text-gray-600 text-[15px]">
              Мапа Пунктів Незламності та Укриттів
            </p>
            <div className="flex gap-2 mt-1">
              <div className="bg-blue-500 rounded-full p-2">
                <div className="w-6 h-6 flex items-center justify-center">
                  <div className="w-3 h-3 bg-white rounded-full"></div>
                </div>
              </div>
              <div className="bg-yellow-400 rounded-full p-2">
                <div className="w-6 h-6 flex items-center justify-center">
                  <div className="w-3 h-3 bg-black rounded-sm rotate-45"></div>
                </div>
              </div>
            </div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="lucide lucide-arrow-right absolute right-5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity"
            >
              <path d="M5 12h14"></path>
              <path d="m12 5 7 7-7 7"></path>
            </svg>
          </div>

          {/* Сітка послуг */}
          <div className="grid grid-cols-3 gap-4">
            <div className="flex flex-col items-center gap-2 cursor-default">
              <div className="bg-black rounded-2xl p-4 w-full aspect-square flex items-center justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="lucide lucide-qr-code w-8 h-8 text-white"
                >
                  <rect width="5" height="5" x="3" y="3" rx="1"></rect>
                  <rect width="5" height="5" x="16" y="3" rx="1"></rect>
                  <rect width="5" height="5" x="3" y="16" rx="1"></rect>
                  <path d="M21 16h-3a2 2 0 0 0-2 2v3"></path>
                  <path d="M21 21v.01"></path>
                  <path d="M12 7v3a2 2 0 0 1-2 2H7"></path>
                  <path d="M3 12h.01"></path>
                  <path d="M12 3h.01"></path>
                  <path d="M12 16v.01"></path>
                  <path d="M16 12h1"></path>
                  <path d="M21 12v.01"></path>
                  <path d="M12 21v-1"></path>
                </svg>
              </div>
              <span className="text-center text-[13px] leading-tight">
                Сканувати QR-код
              </span>
            </div>

            <div className="flex flex-col items-center gap-2 cursor-default">
              <div className="bg-black rounded-2xl p-4 w-full aspect-square flex items-center justify-center">
                <svg
                  width="40"
                  height="68"
                  viewBox="0 0 40 68"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M17.931 64.848C14.4828 62.2187 11.9397 58.2963 11.1207 53.7273H0V5.667C5.9914 8.6411 10.2586 15.236 10.7759 22.0463L12.4569 35.5377L11.6379 35.4084C9.8276 35.4084 8.2759 37.0894 8.2759 38.7704C8.2759 40.3222 9.4828 41.6584 11.0345 42.0032L13.0603 42.4773C16.4224 36.1411 18.319 29.9342 18.319 23.8135C18.319 18.2963 17.5862 12.8222 17.5 7.2187C17.5 4.7187 18.4483 2.4342 20 0.667C21.5517 2.4773 22.5 4.7187 22.5 7.2187C22.5 12.8222 21.681 18.3394 21.681 23.8135C21.681 29.8911 23.5776 36.1411 26.9397 42.4773L28.9655 42.0032C30.5172 41.6584 31.7241 40.3222 31.7241 38.7704C31.7241 37.0894 30.1724 35.4084 28.3621 35.4084L27.5431 35.5377L29.2241 22.0463C30.0431 15.236 34.0086 8.6411 40 5.667V53.7273H28.8793C28.0603 58.2532 25.6465 62.348 22.069 64.848C21.25 65.4515 20.5172 66.1842 20.0431 67.1325C19.4828 66.1411 18.75 65.4515 17.931 64.848ZM5.2586 37.1325C5.7328 35.3222 7.069 33.7704 8.7069 32.8222L7.3707 21.7015C6.8966 18.2532 5.4741 15.1498 3.319 12.6066V37.0894H5.2586V37.1325ZM10.9052 50.3653C10.9052 48.6842 11.1638 47.1325 11.5086 45.5808L9.9569 45.236C7.6724 44.5032 5.9052 42.736 5.3017 40.4515H3.4052V50.3653H10.9052ZM18.2759 50.3653C18.2759 48.3394 16.7241 46.5291 14.6983 46.3135C14.3535 47.6066 14.0948 48.9429 14.0948 50.3653H18.2759ZM18.2759 53.7273H14.569C15.1724 56.3566 16.4655 58.7273 18.2759 60.7963V53.7273ZM23.5345 43.3394C22.1121 40.7101 20.7759 37.9515 19.9569 35.1066C19.1379 37.9946 17.8017 40.7101 16.3793 43.3394C17.8017 43.6842 19.0086 44.6325 19.9569 45.7101C20.9052 44.6325 22.1121 43.6842 23.5345 43.3394ZM25.819 50.3653C25.819 48.9429 25.5603 47.6066 25.2155 46.3135C23.1897 46.5722 21.6379 48.3394 21.6379 50.3653H25.819ZM25.3448 53.7273H21.6379V60.7963C23.4483 58.7273 24.7414 56.3566 25.3448 53.7273ZM36.5948 50.3653V40.4515H34.6983C34.0948 42.736 32.3276 44.5032 30.0431 45.236L28.4914 45.5808C28.8362 47.1325 29.0948 48.6842 29.0948 50.3653H36.5948ZM36.5948 37.1325V12.6498C34.4397 15.1498 32.8879 18.2532 32.5431 21.7446L31.2069 32.8653C32.8879 33.8135 34.181 35.3653 34.6552 37.1756H36.5948V37.1325Z"
                    fill="white"
                  ></path>
                </svg>
              </div>
              <span className="text-center text-[13px] leading-tight">
                Військові облігації
              </span>
            </div>

            <div className="flex flex-col items-center gap-2 cursor-default">
              <div className="bg-black rounded-2xl p-4 w-full aspect-square flex items-center justify-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="lucide lucide-target w-8 h-8 text-white"
                >
                  <circle cx="12" cy="12" r="10"></circle>
                  <circle cx="12" cy="12" r="6"></circle>
                  <circle cx="12" cy="12" r="2"></circle>
                </svg>
              </div>
              <span className="text-center text-[13px] leading-tight">
                Відстуній зв&#x27;язок
              </span>
            </div>
          </div>

          {/* Блок єПідтримка */}
          <div className="relative w-full aspect-[2/1] rounded-[32px] overflow-hidden group cursor-default">
            <div className="absolute inset-0 bg-gradient-to-r from-black via-purple-900 to-orange-500"></div>
            <div className="absolute inset-0 p-6 flex flex-col justify-between">
              <div className="space-y-1">
                <p className="text-[42px] font-medium text-white leading-tight">
                  1000 грн
                </p>
                <p className="text-2xl text-white">Зимова єПідтримка</p>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-2xl text-white">Подати заяву</p>
                <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-arrow-right w-6 h-6"
                  >
                    <path d="M5 12h14"></path>
                    <path d="m12 5 7 7-7 7"></path>
                  </svg>
                </div>
              </div>
            </div>
          </div>
          
          {/* Блок Повідомляйте про відсутній зв'язок */}
          <div className="w-full rounded-[32px] overflow-hidden bg-gradient-to-br from-[#B8FAE6] to-[#D1F5FF]">
            <div className="bg-white/80 backdrop-blur-sm mx-4 mt-4 rounded-xl p-4">
              <p className="text-[15px] leading-tight mb-4">
                Не можете подзвонити або скористатись мобільним інтернетом?
                Повідомте про це, і Дія передасть інформацію регулятору
                зв&#x27;язку (НКЕК).
              </p>
              <div className="flex gap-2">
                <button className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border border-input hover:text-accent-foreground px-4 py-2 flex-1 rounded-full h-11 bg-white hover:bg-white/90">
                  До укриттів
                </button>
                <button className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 text-primary-foreground px-4 py-2 flex-1 rounded-full h-11 bg-black hover:bg-black/90">
                  Продовжити
                </button>
              </div>
            </div>
            <div className="p-4 bg-white/80 backdrop-blur-sm">
              <p className="text-gray-500 mb-1">Сьогодні, 16:40</p>
              <h3 className="text-[22px] leading-tight">
                Повідомляйте про відсутній зв&#x27;язок у Дії
              </h3>
            </div>
          </div>

          {/* Блок Популярні послуги */}
          <div className="space-y-4">
            <h2 className="text-2xl font-medium">Популярні послуги</h2>
            <div className="bg-white/80 backdrop-blur-sm rounded-[32px] divide-y divide-gray-200">
              <div className="flex items-center justify-between p-4 hover:bg-black/5 cursor-default">
                <span className="text-xl">Реєстрація пошкодженого майна</span>
                <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center flex-shrink-0">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-arrow-right w-5 h-5 text-white"
                  >
                    <path d="M5 12h14"></path>
                    <path d="m12 5 7 7-7 7"></path>
                  </svg>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 hover:bg-black/5 cursor-default">
                <span className="text-xl">Заміна водійського посвідчення</span>
                <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center flex-shrink-0">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-arrow-right w-5 h-5 text-white"
                  >
                    <path d="M5 12h14"></path>
                    <path d="m12 5 7 7-7 7"></path>
                  </svg>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 hover:bg-black/5 cursor-default">
                <span className="text-xl">Податки ФОП</span>
                <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center flex-shrink-0">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-arrow-right w-5 h-5 text-white"
                  >
                    <path d="M5 12h14"></path>
                    <path d="m12 5 7 7-7 7"></path>
                  </svg>
                </div>
              </div>
            </div>
          </div>
        </div>
      </main>

      {/* Навігаційна панель */}
      <nav className="fixed bottom-0 left-0 right-0">
        <div className="flex justify-around items-center bg-black text-white h-[80px] pb-[20px] text-[10px]">
          {/* Link 1: Стрічка (Активна сторінка) */}
          <Link className="flex flex-col items-center gap-1 w-1/4" href="/home">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="lucide lucide-align-left text-white"
            >
              <path d="M15 12H3"></path>
              <path d="M17 18H3"></path>
              <path d="M21 6H3"></path>
            </svg>
            <span>Стрічка</span>
          </Link>

          {/* Link 2: Документи */}
          <Link className="flex flex-col items-center gap-1 w-1/4" href="/documents">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="lucide lucide-file-text text-white"
            >
              <path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"></path>
              <path d="M14 2v4a2 2 0 0 0 2 2h4"></path>
              <path d="M10 9H8"></path>
              <path d="M16 13H8"></path>
              <path d="M16 17H8"></path>
            </svg>
            <span>Документи</span>
          </Link>

          {/* Link 3: Сервіси */}
          <Link className="flex flex-col items-center gap-1 w-1/4" href="/services">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="lucide lucide-zap text-white"
            >
              <path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"></path>
            </svg>
            <span>Сервіси</span>
          </Link>

          {/* Link 4: Меню */}
          <Link className="flex flex-col items-center gap-1 w-1/4" href="/menu">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="lucide lucide-user text-white"
            >
              <path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path>
              <circle cx="12" cy="7" r="4"></circle>
            </svg>
            <span>Меню</span>
          </Link>
        </div>
      </nav>
    </>
  );
}